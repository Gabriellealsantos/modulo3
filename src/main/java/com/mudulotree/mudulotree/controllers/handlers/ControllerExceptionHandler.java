package com.mudulotree.mudulotree.controllers.handlers;
...
import com.mudulotree.mudulotree.dto.CustomError;
import com.mudulotree.mudulotree.dto.ValidationError;
...
public class ControllerExceptionHandler {
    ...
    public ResponseEntity<CustomError> entityNotFound(EntityNotFoundException e, HttpServletRequest request) {
        HttpStatus status = HttpStatus.NOT_FOUND;
        CustomError err = new CustomError(Instant.now(), status.value(), "Cliente Inexistente", request.getRequestURI());
        return ResponseEntity.status(status).body(err);
    }
